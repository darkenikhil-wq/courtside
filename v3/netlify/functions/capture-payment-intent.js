exports.handler = async (event) => {
  ...
  if (intent.body.status !== 'requires_capture' || intent.body.metadata?.purpose !== 'court_booking') {
    return jsonResponse(409, {
      ...
      message: 'Stripe authorization is not ready for court-booking capture.',
      payment: {
  ...
  const stripeRes = await fetch(`https://api.stripe.com/v1/payment_intents/${encodeURIComponent(paymentIntentId)}/capture`, {
    method: 'POST',
    headers: {
  ...
  if (!stripeRes.ok) {
    return jsonResponse(stripeRes.status, {
      status: 'stripe_error',
      code: payload.error && payload.error.code || 'STRIPE_CAPTURE_ERROR',
      message: payload.error && payload.error.message || 'Stripe could not capture the authorization.',
    });
  }
